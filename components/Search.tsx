...
export const Search: FC<SearchProps> = ({ onSearch, onAnswerUpdate, onDone }) => {
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("LUCIDITY_KEY", apiKey);
  ...
  const handleClear = () => {
    localStorage.removeItem("LUCIDITY_KEY");

    setApiKey("");
  };

  useEffect(() => {
    const LUCIDITY_KEY = localStorage.getItem("LUCIDITY_KEY");

    if (LUCIDITY_KEY) {
      setApiKey(LUCIDITY_KEY);
    } else {
      ...
      {loading ? (
        ...
        <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center space-y-6 px-3 pt-32 sm:pt-64">
          <div className="flex items-center">
            <IconBolt size={36} />
            <div className="ml-1 text-center text-4xl">Lucidity</div>
          </div>
